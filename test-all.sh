...
grep -s -q "^#include \"SneakyRooks\.h\"" SneakyRooks.c
grep_val=$?

if [[ $grep_val != 0 ]]; then
	echo ""
	echo "  Whoa, buddy! Your SneakyRooks.c file does not appear to have"
	echo "  a proper #include statement for SneakyRooks.h. Please read"
	echo "  Section 3 of the assignment PDF before proceeding."
	echo ""
	exit
fi
...
function run_test_case()
{
	...
	gcc SneakyRooks.c $testcase_file 2> /dev/null
	compile_val=$?
...
if ! [ -x "$(command -v grep)" ]; then
	...
	num_spc_lines=`grep "^ " SneakyRooks.c | wc -l | awk '{$1=$1};1'`
	num_tab_lines=`grep "$(printf '^\t')" SneakyRooks.c | wc -l | awk '{$1=$1};1'`
	num_und_lines=`grep "$(printf '^[^\t ]')" SneakyRooks.c | wc -l | awk '{$1=$1};1'`
	...
	if [ "$num_spc_lines" -gt 0 ] && [ "$num_tab_lines" -gt 0 ]; then
		...
		echo ""
		echo "     grep \"^ \" SneakyRooks.c -n"
		echo "     grep \"\$(printf '^\t')\" SneakyRooks.c -n"
